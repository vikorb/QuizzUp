...
app.post("/auth/login", async (req, reply) => {
  ...
  const q = db("admins")
    .select("id", "company_id", "role", "firstname", "lastname", "username", "email", "mdp_hash", "status", "deleted_at")
    .where("status", 1)
    .whereNull("deleted_at")

  if (isEmail) q.andWhereRaw("lower(email) = lower(?)", [identifier])
  else q.andWhereRaw("lower(username) = lower(?)", [identifier])

  const admin = await q.first()
...
app.get("/me", { preHandler: [async (req) => req.jwtVerify()] }, async (req, reply) => {
  ...
  const admin = await db("admins")
    .select("id", "company_id", "role", "firstname", "lastname", "username", "email", "status", "created_at", "updated_at")
    .where({ id: adminId })
    .first()
